...
class TeamMembersController extends Controller
{
    ...
    protected function init()
    {
        ...
        $this->ajaxValidatorRules['reSendInvitation'] = [
            'team_member_id' => 'required|exists:users,id,deleted_at,NULL',
        ];
    }
    ...
    public function getData()
    {
        ...
        return Datatables::of($user)
            ...
            ->addColumn('has_password', function ($user) {
                return (bool)$user->password;
            })
            ->addColumn('token_expired', function ($user) {
                if (!$user->password) {
                    ...
                    return Carbon::parse($expireToken->created_at)
    ...
    public function store(): HttpResponse
    {
        if (!$this->currentUser->canAddTeamMembers()) {
            return response([
                'message' => __('You have reached the limit of the adding new team members'),
    ...
    /**
     * Re-sent invitation with new password token to the team member
     *
     * @param Request $request with team_member_id
     *
     * @return HttpResponse with message and {@see Response::HTTP_NOT_FOUND} response code
     *                      if team member does not exist in the owner's team,
     *                      otherwise {@see Response::HTTP_OK} response code and success message.
     */
    public function reSendInvitation(Request $request): HttpResponse
    {
        $owner = auth()->user();

        if (
            !OwnerToTeamMember::where('owner_id', $owner->id)
            ->where('team_member_id', $request->team_member_id)
            ->exists()
        ) {
            return response(
                ['message' => __('The team member does not exist in your team')],
        ...
        app(User::class)->sendInvitationMailToTeamMember($owner, $teamMember);

        return response(['message' => __('The invitation has been successfully resent')]);
    }
}
