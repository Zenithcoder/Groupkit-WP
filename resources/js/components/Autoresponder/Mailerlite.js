...
class Mailerlite extends Component{
    ...
    componentDidMount(){
        var response=this.props.setActiveAutoresponder('Mailerlite')
    ...
    saveAutoresponder(){
        var obj={
            responder_type:'Mailerlite',
            responder_json:{
                activeList:this.state.activeList,
                api_key:this.state.mailerlite.api_key
            },
    ...
    handleChang(param,event){
        this.state.mailerlite[param]=event.target.value
        this.setState({mailerlite:this.state.mailerlite});
    }
    ...
    getMailerlite(){
        API.mailerlite(this.state.mailerlite).then((res) => {
            if(res.data.code==200){
                this.setState({
        ...
        return (
            <div className="">
                <form className="forms-sample" id="autoresponder_from">
                    <label className="col-sm-12 col-form-label pl-0">Enter Your Mailerlite API Key</label>
                    <div className="form-group row">
                        <div className="col-sm-12">
                            <input type="text" className="form-control"  placeholder="Mailerlite API Key" value={this.state.mailerlite.api_key} onChange={this.handleChang.bind(this,"api_key")} />
                        </div>
                    </div>
                    ...
                    <div className="form-group row" style={this.state.activeList ? {display:'none'} : {display:'block'}}>
                        <div className="col-sm-12">
                            <button type="button" className="btn btn-primary" onClick={this.getMailerlite.bind(this)} disabled={this.state.activeList ? 'disabled' : ''}>
                                CHECK
                            </button>
                        </div>
                    </div>
